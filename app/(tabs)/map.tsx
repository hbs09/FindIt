...
// Estilo Claro (Oculta POIs como querias)
const lightMapStyle = [
...
// Estilo Escuro (Oculta POIs e escurece ruas/fundos)
const darkMapStyle = [
...
export default function MapScreen() {
  ...
  // 1. Hook de Tema e Estilos Dinâmicos
  const { colors, isDarkMode } = useTheme();
  const styles = useMemo(() => createStyles(colors, isDarkMode), [colors, isDarkMode]);
  const activeMapStyle = isDarkMode ? darkMapStyle : lightMapStyle;
  ...
  // Render do Cluster agora movido para dentro para usar os estilos dinâmicos
  ...
  return (
    <View style={styles.container}>
      <ClusteredMapView
        ...
        customMapStyle={activeMapStyle} // <-- Isto faz o Google Maps (Android) ficar escuro
        userInterfaceStyle={isDarkMode ? "dark" : "light"} // <--- ADICIONA ISTO PARA O iOS (Apple Maps) FICAR ESCURO!
        initialRegion={{
              ...
              <View style={styles.customMarker}>
                <View style={[
                  ...
                  {hasImage ? (
                    ...
                    <Ionicons name={getCategoryIcon(salao.categoria)} size={20} color={colors.bg} />
                  )}
                </View>
      ...
      {loading && (
        <View style={styles.searchAreaContainer}>
          <View style={styles.searchAreaBtn}>
            <ActivityIndicator size="small" color={colors.bg} style={{ marginRight: 6 }} />
      ...
      <View style={styles.controlsContainer}>
        ...
        {sheetState === 'closed' && saloesVisiveis.length > 0 && (
          ...
          >
            <Ionicons name="list" size={18} color={colors.bg} />
...
const createStyles = (colors: any, isDarkMode: boolean) => StyleSheet.create({
  ...
  markerImageContainer: {
    ...
    backgroundColor: colors.text, borderWidth: 2, borderColor: colors.bg,
    justifyContent: 'center', alignItems: 'center', overflow: 'hidden',
    shadowColor: "#000", shadowOffset: { width: 0, height: 2 }, shadowOpacity: 0.25, elevation: 5,
  },
  markerNoImageContainer: { backgroundColor: colors.text, borderColor: colors.border },
  ...
  markerArrowNoImage: { borderTopColor: colors.text },
  ...
  searchBar: {
    flexDirection: 'row', backgroundColor: colors.card, borderRadius: 25,
    paddingHorizontal: 16, height: BTN_SIZE, alignItems: 'center',
    shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.1, shadowRadius: 10, elevation: 5,
    borderWidth: 1, borderColor: colors.border,
  },
  input: { flex: 1, fontSize: 15, color: colors.text, marginRight: 10 },
  ...
  searchAreaText: { fontWeight: '600', color: colors.bg, fontSize: 13 },
  controlsContainer: {
    position: 'absolute', right: 20, bottom: 120, alignItems: 'flex-end', gap: 15, zIndex: 10,
  },
  circleBtn: {
    width: BTN_SIZE, height: BTN_SIZE, borderRadius: BTN_SIZE / 2, backgroundColor: colors.card,
  ...
  listPillText: { color: colors.bg, fontWeight: '600', fontSize: 14 },
  bottomSheet: {
    position: 'absolute', bottom: 0, width: '100%', backgroundColor: colors.bg,
  ...
  sheetHeader: {
    alignItems: 'center', paddingVertical: 15, backgroundColor: colors.card, borderBottomWidth: 1, borderBottomColor: colors.border,
  },
  handleIndicator: { width: 40, height: 5, backgroundColor: colors.border, borderRadius: 3, marginBottom: 8 },
  sheetTitle: { fontWeight: '600', fontSize: 13, color: colors.subText },
  card: {
  ...
  cardImage: { width: 80, height: '100%', borderRadius: 12, backgroundColor: colors.iconBg },
  cardContent: { flex: 1, marginLeft: 12, justifyContent: 'center' },
  cardTitle: { fontSize: 16, fontWeight: '700', color: colors.text, marginBottom: 4 },
  badgeRow: { flexDirection: 'row', marginBottom: 6 },
  categoryBadge: { backgroundColor: colors.iconBg, paddingHorizontal: 8, paddingVertical: 2, borderRadius: 6 },
  categoryBadgeText: { fontSize: 11, color: colors.text, fontWeight: '600' },
  locationRow: { flexDirection: 'row', alignItems: 'center', gap: 4 },
  cardLocation: { fontSize: 13, color: colors.subText },

  clusterContainer: {
    width: 34,
    height: 34,
    borderRadius: 20,
    backgroundColor: colors.text, // Preto no Light Mode, Branco no Dark Mode
